    ...
    function Plot( target_, data_, options_ ) {
        ...
        function insertAxisLabels() {
            if( options.xaxis.label ) {
                ...
                target.append( "<div class='xaxislabel' style='color:" +
            ...
            if( options.yaxis.label ) {
                ...
                var yAxisLabel = $("<div class='yaxislabel' style='color:" +
        ...
        function drawSeries( series, index ) {
            if( series.lines.show || ( !series.bars.show && !series.points.show && !series.deltas.show ) ) {
                drawSeriesLines( series );
            }
            if( series.bars.show )    drawSeriesBars( series, index );
        ...
        function drawSeriesBars( series, series_index ) {
            function plotBars( data, barWidth, offset, fill ) {
                for( var i = 0; i < data.length; i++ ) {
                    ...
                    // draw label
                    if (series.bars.labelFormatter) {
                      klass = 'barLabel-' + series_index + '-' + i;
                      if (target.find('.' + klass).size() == 0) {
                        html = '<div style="position:absolute;top:' +
                                ( plotOffset.top + tVert( top ) + options.grid.labelMargin ) +
                                'px;left:' + ( plotOffset.left + tHoz( x ) - xaxis.labelWidth / 2) +
                                'px;width:' + xaxis.labelWidth + 'px;text-align:center" class="' + klass  + ' barLabel">' +
                                 series.bars.labelFormatter(data[i]) + "</div>";
            ...
            for( var i = 0; i < series.length; ++i ) {
                ...
                for( var j = start; j < end; ++j ) {
                    ...
                    if (options.bars) {
                      var x = data[j].x,
                          y = data[j].y;
                      var halfBar = options.bars.barWidth / 2;
                      var left = x - halfBar,
                          right = x + halfBar,
                          bottom = ( y < 0 ? y : 0 ),
                          top = ( y < 0 ? 0 : y );

                      var hitsOnXAxis = mouseX >= tHoz( left ) && mouseX <= tHoz( right );
                      var hitsOnYAxis = mouseY >= tVert( top ) && mouseY <= tVert( bottom );
                      if (hitsOnXAxis && hitsOnYAxis) {
                        selectedItem = data[j];
        ...
        function highlight( marker, highlightOptions ) {
            ...
            if( marker ) {
                ...
                if (options.bars) {
                  ...
                  var temp_series = {
                      ...
                      lines: { show: false },
                      bars: $.extend( true, options.bars, highlightOptions),
                      color: highlightOptions.color || color,
                      data: [ { x: marker.x, y: marker.y } ]
                  };
                  draw();
                  drawSeriesBars( temp_series );
                } else {
                  var temp_series = {
                      shadowSize: options.shadowSize,
                      lines: { show: false },
